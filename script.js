//Сегодня я хочу
//— ускорить вычисления за счёт перехода к плотной упаковке вселенной (каждая ячейка может содержать только одну клетку.
//— Продумать структуру, которая поможет организмам соревноваться.
//
//v Подправлю цвета
...
function World(height, width) {
  ...
  this.parse = function(string){
    ...
    return([ 72,	141, 738,	756,	484,	35,	947, 157,	538,	223,	507,	757, 995,	175,	82,	263,	270, 533,	372,	19,	912,	708, 836,	355,	432,	176,	349, 621,	242,	301,	825,	533, 871,	129,	605,	270,	491, 691,	73,	308,	951,	927, 599,	245,	647,	937,	65, 413,	672,	2,	995,	966, 741,	691,	971,	558,	596, 622,	723,	526,	532,	590, 652,	597,	306,	371,	191, 278,	641,	385,	407,	947, 121,	234,	186,	14,	577, 950,	585,	995,	127,	600, 616,	459,	558,	549,	274, 670,	565,	106,	870,	830, 241,	634,	133,	516,	790 ]); // cascade
    ...
    //return([550,550,550,550,550,550,550,550,550,550,550,550,550]);
  };
...
function Animal(dna,x,y){//TODO Animal is more wordish now
  this.boxes = {};
  this.boxes['1'] = {}
  this.boxes['2'] = {}
  this.boxes['3'] = {}
  ...
  this.boxes['4']['4'] = 3;
  this.boxes['1']['4'] = 3;
  //console.log(this.boxes.filter(function(d){
    //return true;
    //return '2' in obj;
    //}));
...
function Cell(dna,cursor,x,y){
  ...
  this.colorUpdate = function(){
    //this.color = "hsl("+parseInt(360 * this.cursor / this.dna.length)+", 100%, 50%)";
    //this.color = "hsl("+700+parseInt(60 * this.cursor / this.dna.length)+", 100%, 50%)";
    this.color = "hsl("+100+parseInt(160 * this.cursor / this.dna.length)+", 100%, 50%)";
    //console.log(parseInt(100 * this.cursor / this.dna.length));
  };
  this.colorUpdate();
  this.develop = function(){
    this.normalizeCursor();
...
var world = new World(10000, 10000);
